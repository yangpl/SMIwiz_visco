...
void write_vector(const char *name, const float *x, int n)
{
  ...
  if(fp==NULL) err("cannot open %s for writing", name);
  if(fwrite(x, sizeof(float), (size_t)n, fp)!=(size_t)n)
    err("cannot write %s", name);
  if(fclose(fp)!=0) err("cannot close %s", name);
}

void append_vector(const char *name, const float *x, int n)
{
  FILE *fp = fopen(name, "ab");
  if(fp==NULL) err("cannot open %s for appending", name);
  if(fwrite(x, sizeof(float), (size_t)n, fp)!=(size_t)n)
    err("cannot append %s", name);
  if(fclose(fp)!=0) err("cannot close %s", name);
}
