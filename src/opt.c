...
/*< save current model and gient >*/
{
  ...
  if(opt->kpair < opt->npair){
    ...
    for(i=0; i < opt->npair-1; i++){
      memcpy(sk[i], sk[i+1], n*sizeof(float)); //sk[i]<-sk[i+1]
      memcpy(yk[i], yk[i+1], n*sizeof(float)); //yk[i]<-yk[i+1]
    }
...
void lbfgs_descent2(int n, float *q, float *rho, float *alpha,
		    float **sk, float **yk, opt_t *opt)
...
{
  int i,k;
  double rr, rrnew, rr0, rzold, rznew, pAp, alp, beta;

  float *r = alloc1float(n);
  float *z = alloc1float(n);
  float *p = alloc1float(n);
  float *Ap = alloc1float(n);
  /* Assume d0=0, so r0=b-A*d0=-g. */
  ...
  for(i=0; i<n; i++){
    p[i] = z[i];
    rzold += (double)r[i]*(double)z[i];
  }
  if(!(rzold>0.0) || !isfinite(rzold))
    err("Newton-CG preconditioner is not positive definite");
  for(k=0; k<opt->ncg; k++){
    Hv(x, p, Ap);//compute Ap

    pAp = 0;
    for(i=0; i<n; i++) pAp += (double)p[i]*(double)Ap[i];
    ...
    for(i=0; i<n; i++){
      d[i] += alp*p[i];
      r[i] -= alp*Ap[i];
      rrnew += (double)r[i]*(double)r[i];
    }

    if(rrnew<=opt->tol_cg*opt->tol_cg*rr0) break;

    memcpy(z, r, n*sizeof(float));
    if(P!=NULL) P(z);
    rznew = 0.0;
    for(i=0; i<n; i++) rznew += (double)r[i]*(double)z[i];
    if(!(rznew>0.0) || !isfinite(rznew))
      err("Newton-CG preconditioner is not positive definite");
    beta = rznew/rzold;
